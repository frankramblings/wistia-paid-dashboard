  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-w-hi">YouTube Ads</h1>
      ...
      <div className="bg-w-surface border border-w-border rounded-lg shadow-card overflow-hidden mb-8">
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {ads.map(ad => (
                <tr key={ad.adName} className="border-b border-w-border last:border-0 hover:bg-w-canvas">
                  <td className="py-3 px-5">
                    <div className="text-w-hi font-medium">{ad.adName}</div>
                    <div className="text-w-mid text-xs">{ad.creator}</div>
                  </td>
                  <td className="px-5 text-w-mid whitespace-nowrap">{ad.format}</td>
                  <td className={`px-5 whitespace-nowrap ${scoreAndColor('youtube', 'played100', ad.played100)}`}>{ad.played100.toFixed(1)}%</td>
                  <td className={`px-5 whitespace-nowrap ${scoreAndColor('youtube', 'interactionRate', ad.interactionRate)}`}>{ad.interactionRate.toFixed(1)}%</td>
                  <td className={`px-5 whitespace-nowrap ${scoreAndColor('youtube', 'avgCPV', ad.avgCPV)}`}>${ad.avgCPV.toFixed(3)}</td>
                  <td className="px-5 text-w-mid whitespace-nowrap tabular-nums">{ad.earnedSubs || '—'}</td>
                  <td className="px-5 text-w-mid whitespace-nowrap tabular-nums">{ad.earnedSubs > 0 ? `$${ad.costPerConv.toFixed(2)}` : '—'}</td>
                  <td className="px-5">{ad.evaluation && <BenchmarkBadge status={ad.evaluation.status} signals={ad.evaluation.signals} />}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <h2 className="text-xs font-semibold text-w-mid uppercase tracking-wider mb-3">Creator Comparison</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {creators.map(creator => {
          ...
          return (
            <div key={creator} className="bg-w-surface rounded-lg p-5 border border-w-border shadow-card">
              <div className="font-semibold text-w-hi mb-3">{creator}</div>
