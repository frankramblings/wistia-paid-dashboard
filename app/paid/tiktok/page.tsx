...
export default function TikTokAdsPage() {
  ...
  return (
    <div>
      <div className="mb-2">
        <h1 className="font-bold text-w-hi">TikTok Ads</h1>
      </div>
      ...
      {notConfigured && (
        <div className="mb-6 px-4 py-3 rounded-xl bg-w-blue-bg border border-w-blue/30 text-w-blue text-sm">
              ...
              {campaigns.map((c, i) => {
                ...
                return (
                  <tr key={c.campaignId} className={`${i % 2 === 0 ? 'bg-[#f9f9fb]' : 'bg-white'} border-b border-[#ebebed] last:border-0 hover:bg-[#eeeef0] transition-colors`}>
                    <td className="py-4 px-4 text-w-hi">{c.name}</td>
                    <td className="py-4 px-4 text-w-hi whitespace-nowrap tabular-nums">{c.videoViews.toLocaleString()}</td>
                    <td className={`py-4 px-4 whitespace-nowrap tabular-nums ${scoreAndColor('tiktok', 'viewRate', viewRate)}`}>{viewRate.toFixed(1)}%</td>
                    <td className={`py-4 px-4 whitespace-nowrap tabular-nums ${cpv > 0 ? scoreAndColor('tiktok', 'cpv', cpv) : 'text-w-mid'}`}>{cpv > 0 ? `$${cpv.toFixed(3)}` : '—'}</td>
                    <td className={`py-4 px-4 whitespace-nowrap tabular-nums ${scoreAndColor('tiktok', 'cpm', cpm)}`}>${cpm.toFixed(2)}</td>
                    <td className="py-4 px-4 text-w-hi whitespace-nowrap tabular-nums">${c.spend.toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                    <td className={`py-4 px-4 whitespace-nowrap tabular-nums ${scoreAndColor('tiktok', 'ctr', c.ctr)}`}>{c.ctr.toFixed(2)}%</td>
                    <td className={`py-4 px-4 whitespace-nowrap tabular-nums ${c.cpc > 0 ? scoreAndColor('tiktok', 'cpc', c.cpc) : 'text-w-mid'}`}>{c.cpc > 0 ? `$${c.cpc.toFixed(2)}` : '—'}</td>
                  </tr>
                );
              })}
            </tbody>
